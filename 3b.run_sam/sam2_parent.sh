...
for model in "${models[@]}" ; do
    for downscale_factor in "${downscale_factors[@]}" ; do
        ...
        if [ "$downscale_factor" -lt 10 ]; then
            sbatch --nodes=1 --mem=240G --time=22:00:00 --partition=aa100 --gres=gpu:1 --constraint=gpu80 --output=alpine_std_out_std_err-%j.out run_sam2_HPC.sh "$model" "$downscale_factor"
            echo "Submitted job for downscale factor $downscale_factor with model $model on the 80 GPU partition."
        else
            sbatch --nodes=1 --mem=240G --time=22:00:00 --partition=aa100 --gres=gpu:1 --output=alpine_std_out_std_err-%j.out run_sam2_HPC.sh "$model" "$downscale_factor"
